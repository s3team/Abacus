...
//    AJ_ASSERT((len % 16) == 0);
    ...
    while (len) {
        ...
        memset((uint8_t*)xorbuf, 0, sizeof(xorbuf));
...
int main()
{
    ...
    memset(plaintext, 0x02, sizeof(plaintext));
    memset(ciphertext, 0x02, sizeof(ciphertext));

    AJ_AES_Enable(key);     /* Key schedule, updates global AES context */
    AJ_AES_ECB_128_ENCRYPT(key, plaintext, ciphertext);
