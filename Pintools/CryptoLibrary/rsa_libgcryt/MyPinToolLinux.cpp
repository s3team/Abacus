...
std::string function_name, entry_function_name;
FILE *fp;
...
VOID static RTN_entry_start(char *name, char *rtn_name)
{
...
VOID static RTN_start(char *name, char *rtn_name, gcry_mpi_t D, gcry_mpi_t P, gcry_mpi_t Q, gcry_mpi_t U)
{
  if (entry_flag && !capture_flag)
  {
    capture_flag = true;
    uint8_t value;
    // int i = 0;
    ...
    printf("D:  neg: %d, size: %u address: %p\n", D->sign, D->nlimbs, D->d);
    fprintf(fp, "Start; %p; %d; \n", D->d, D->nlimbs * 8 * 4);
    key = (const unsigned char *)D->d;
    for (int i = 0; i < D->nlimbs * 4; i++)
    {
    ...
    fprintf(fp, "Start; %p; %d; \n", P->d, P->nlimbs * 8 * 4);
    key = (const unsigned char *)P->d;
    for (int i = 0; i < P->nlimbs * 4; i++)
    {
      PIN_SafeCopy(&value, key++, sizeof(uint8_t));
      fprintf(fp, "%x; ", value);
      printf("Key:  %d %x \n", i, value);
    ...
    fprintf(fp, "Start; %p; %d; \n", Q->d, Q->nlimbs * 8 * 4);
    key = (const unsigned char *)Q->d;
    for (int i = 0; i < Q->nlimbs * 4; i++)
    {
      PIN_SafeCopy(&value, key++, sizeof(uint8_t));
      fprintf(fp, "%x; ", value);
      printf("Key:  %d %x \n", i, value);
    ...
    printf("U:  neg: %d, size: %u address: %p\n", U->sign, U->nlimbs, U->d);
    fprintf(fp, "Start; %p; %d; \n", U->d, U->nlimbs * 8 * 4);
    key = (const unsigned char *)U->d;
    for (int i = 0; i < U->nlimbs * 4; i++)
    {
      PIN_SafeCopy(&value, key++, sizeof(uint8_t));
      fprintf(fp, "%x; ", value);
      printf("Key:  %d %x \n", i, value);
...
VOID Routine(RTN rtn, VOID *v)
{
  ...
  if (rtn_name.find(entry_function_name) != std::string::npos)
  {
    RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)RTN_entry_start,
                   IARG_ADDRINT, entry_function_name.c_str(),
...
int main(int argc, char *argv[])
{
  ...
  entry_function_name = "rsa_decrypt";
  function_name = "secret_core_crt";
