...
// We don't combine rtn_recv and rtn_send together because we need compilers try to make them
// void crypto_poly1305(u8 mac[16], const u8 *message, size_t message_size, const u8 key[32])
VOID static RTN_start(char *name, char *rtn_name, uint8_t *mac, char *input_text,
                      size_t text_size, char *key)
{
    if (start_ins == false && first_time)
    {
        ...
        while (i < key_bytes)
        {
            PIN_SafeCopy(&value, mac++, sizeof(uint8_t));
            fprintf(fp, "%x; ", value);
            printf("mac:  %d %x \n", i, value);
            ++i;
        }
