...
namespace tana{
    namespace ELF {
        ...
        public:
            ...
            Symbol(const std::string &s_name, ELFIO::Elf64_Addr s_value, ELFIO::Elf_Xword s_size, unsigned char s_bind, \
               unsigned char s_type, ELFIO::Elf_Half s_section_index, unsigned char s_other) :
                    name(s_name), value(s_value), size(s_size), bind(s_bind), type(s_type),
                    section_index(s_section_index),
                    other(s_other) {}
        };
