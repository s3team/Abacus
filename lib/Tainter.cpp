...
//Immediate to Register
    void
    Tainter::spreadTaintImediate2Register(x86::x86_reg reg) {
        untaint(reg);
    }
...
//Memory to register
    ...
    void
    Tainter::taintIns(Inst_Base &ins) {
        using namespace x86;
        ...
        switch (opcode) {
            case x86_insn::X86_INS_MOV:
                if (((ins.oprd[0])->type == Operand::Reg) && ((ins.oprd[1])->type == Operand::Reg)) {
                ...
                if (((ins.oprd[0])->type == Operand::Mem) && ((ins.oprd[1])->type == Operand::Reg)) {
                    std::cout << "MEM REG" << std::endl;
                    auto reg1 = Registers::convert2RegID((ins.oprd[1])->field[0]);
                    spreadTaintRegister2Memory(ins.addrn, reg1, ins.memory_address);

                    break;
                }

                if (((ins.oprd[0])->type == Operand::Reg) && ((ins.oprd[1])->type == Operand::Mem)) {
                    std::cout << "REG Mem" << std::endl;
                    auto reg1 = Registers::convert2RegID((ins.oprd[0])->field[0]);
                    spreadTaintMemory2Register(ins.addrn, ins.memory_address, reg1);
                    break;
                }

                if (((ins.oprd[0])->type == Operand::Reg) && ((ins.oprd[1])->type == Operand::ImmValue)) {
                ...
                if (((ins.oprd[0])->type == Operand::Mem) && ((ins.oprd[1])->type == Operand::ImmValue)) {
                ...
                if (((ins.oprd[0])->type == Operand::Mem) && ((ins.oprd[1])->type == Operand::Mem)) {
            ...
            case x86_insn::X86_INS_POP:
                if ((oprnum == 1) && ((ins.oprd[0])->type == Operand::Reg)) {
                    auto reg = Registers::convert2RegID((ins.oprd[0]->field[0]));
                    spreadTaintRegister2Memory(ins.addrn, reg, ins.memory_address);
                    break;
                }
            ...
            default:
                std::cout << "ERROR at" << __FILE__ << ins.get_opcode_operand() << __LINE__ << std::endl;
                break;
